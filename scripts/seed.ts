...
// Next.js carrega automaticamente .env.local, não precisamos do dotenv
...
async function seedArticles() {
  if (!existsSync(ARTICLES_INDEX_PATH)) {
    console.warn(`Arquivo n�o encontrado: ${ARTICLES_INDEX_PATH}`);
    return;
  }
...
async function seedCertificates() {
  ...
  console.info(`Seed de certificados conclu�do (${certificates.length}).`);
}
